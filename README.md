...
- **들여쓰기 레벨 정규화**: 글머리 기호의 들여쓰기를 일관성 있게 조정합니다.
  - 최소 들여쓰기가 2칸이면 모든 글머리의 들여쓰기를 4칸으로 변환합니다.

### 각주 정규화

- **각주 마커 정규화**: `[숫자]` 형식의 각주 마커를 `[^숫자]` 형식(Markdown 각주)으로 변환합니다.
  - 예: `[1]` → `[^1]`
- **각주 정의 정규화**: `[^숫자](URL)` -> `[^숫자]: URL`
  - 예: `[^1](https://example.com)` → `[^1]: https://example.com`
- **미사용 각주 삭제**: 문서에서 참조되지 않는 각주 정의는 자동으로 삭제됩니다.
...
- **특수문자 마커 변환**: 특수문자 마커를 일반 문자로 변환합니다 (header only).
  - 예: `## ① hello` → `## 1) hello`
