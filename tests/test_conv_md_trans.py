...
class TestRemoveNumberedBrackets:
    ...
    def test_remove_numbered_brackets_cases(self, input_text, expected):
        assert remove_numbered_brackets(input_text) == expected


class TestRemoveNumberedLines:
    @pytest.mark.parametrize(
        "input_text,expected",
        [
            pytest.param("", "", id="empty_string"),  # 빈_문자열
    ...
    def test_remove_numbered_lines_cases(self, input_text, expected):
        assert remove_numbered_lines(input_text) == expected


class TestConvertAsteriskToDash:
    @pytest.mark.parametrize(
        "input_text,expected",
        [
            pytest.param("", "", id="empty_string"),  # 빈_문자열
    ...
    def test_convert_asterisk_to_dash_cases(self, input_text, expected):
        assert convert_asterisk_to_dash(input_text) == expected


class TestReduceNumberedListSpaces:
    @pytest.mark.parametrize(
        "input_text,expected",
        [
            pytest.param("", "", id="empty_string"),  # 빈_문자열
    ...
    def test_reduce_numbered_list_spaces_cases(self, input_text, expected):
        assert reduce_numbered_list_spaces(input_text) == expected


class TestAddBlankLineAfterHeaders:
    @pytest.mark.parametrize(
        "input_text,expected",
        [
            pytest.param("", "", id="empty_string"),  # 빈_문자열
    ...
    def test_add_blank_line_after_headers_cases(self, input_text, expected):
        assert add_blank_line_after_headers(input_text) == expected


class TestNormalizeHeaderLevels:
    @pytest.mark.parametrize(
        "input_text,expected",
        [
            pytest.param("", "", id="empty_string"),  # 빈_문자열
            pytest.param("No headers", "No headers", id="no_headers"),  # 헤더_없음
            pytest.param("# H1\n## H2", "# H1\n## H2", id="level1_max_no_change_a"),  # 레벨1_최대_변경없음_a
            pytest.param("# H1", "# H1", id="level1_max_no_change_b"),  # 레벨1_최대_변경없음_b
            pytest.param("### H3", "## H3", id="h3_alone_to_h2"),  # h3_단독_h2로
            pytest.param("#### H4", "## H4", id="h4_alone_to_h2"),  # h4_단독_h2로
            pytest.param("# H1\n### H3", "# H1\n## H3", id="h1_h3_normalized"),  # h1_h3_정규화됨
            pytest.param("## H2\n#### H4", "## H2\n#### H4", id="level2_max_no_change"),  # 레벨2_최대_변경_없음
            pytest.param(
                "### H3\n#### H4\n##### H5", "## H3\n### H4\n#### H5", id="all_levels_normalized"
            ),  # 모든_레벨_정규화됨
        ],
    )
    def test_normalize_header_levels_cases(self, input_text, expected):
        result = normalize_header_levels(input_text)
        assert result == expected

        # Verify max level is at most 2
        header_pattern = re.compile(r"^(#{1,6})\s+", re.MULTILINE)
        headers = header_pattern.findall(result)
        if headers:
            max_level = max(len(h) for h in headers)
            assert max_level <= 2, f"Max level should be <= 2, got {max_level}"


class TestConvertLatexBracketsToDoubleDollar:
    @pytest.mark.parametrize(
        "input_text,expected",
        [
            pytest.param("", "", id="empty_string"),  # 빈_문자열
    ...
    def test_convert_latex_brackets_to_double_dollar_cases(self, input_text, expected):
        assert convert_latex_brackets_to_double_dollar(input_text) == expected


class TestConvertLatexParenthesesToDollar:
    @pytest.mark.parametrize(
        "input_text,expected",
        [
            pytest.param("", "", id="empty_string"),  # 빈_문자열
            ...
            pytest.param(
                "Regular (parentheses)", "Regular (parentheses)", id="regular_parens_unchanged"
            ),  # 일반_괄호_변경없음
    ...
    def test_convert_latex_parentheses_to_dollar_cases(self, input_text, expected):
